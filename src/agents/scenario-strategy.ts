  ...
  validation: {
    ...
    generation: {
      ...
      promptHints: [
        ...
        "Wait for dynamic content before asserting with explicit timeout",
        "For SPA content: wait for the target element to become visible using locator.waitFor() before asserting",
        "For lists/panels: verify items are in the DOM (visible in the list), not just existence",
        "Use regex patterns for partial/flexible text matching in dynamic content",
        "Always add explicit timeouts (15000ms+) for elements that load asynchronously",
      ],
    },
    validation: {
      requiredPatterns: [
    ...
    analysis: {
      commonFailureCauses: [
        ...
        "List/panel items not yet loaded in DOM",
      ],
      fixSuggestions: [
        "Use regex for flexible text matching: /expected text/i",
        "Wait for network response or element visibility before asserting",
        "Prefer toContainText over toHaveText for partial matches",
        "Wait for the target element to become visible using locator.waitFor() before asserting async content",
        "Verify elements in a list are actually visible, not just exist in DOM",
      ],
    },
  },
  ...
  "modal-interaction": {
    ...
    generation: {
      ...
      promptHints: [
        ...
        "Locate the modal/dialog by role and accessible name (e.g. page.getByRole('dialog', { name: /modal name/i })) instead of page.locator('[role=\"dialog\"]').first(), as there may be hidden dialog elements in the DOM",
  ...
  interaction: {
    ...
    generation: {
      ...
      promptHints: [
        ...
        "Use locator.waitFor() to await element rendering between sequential clicks",
        "For SPA sites (e.g. personalization panels, follow flows): wait for the expected UI element response to be visible after each click",
        "For toast notifications: use flexible text matching with regex, e.g. /toast.*text/i",
        "For follow/unfollow buttons: assert the UI state changed (button text, icon, list updated), not just the toast",
        "Use longer timeouts (15000ms+) for personalization/dynamic panel loads",
      ],
    },
    validation: {
      requiredPatterns: [
    ...
    analysis: {
      commonFailureCauses: [
        ...
        "Second click fires before first action settled (missing network wait)",
        "Element obscured by overlay or modal",
        "Toast verification too strict — doesn't match actual DOM structure",
        "Personalization panel takes longer to load than expected timeout",
      ],
      fixSuggestions: [
        "Use getByRole('button', { name: /exact text/i }) for button lookup",
        "Use locator.waitFor() on the next clickable element before attempting to click it",
        "Check for overlaying elements blocking the click target",
        "For follow/unfollow: verify the button state or list content changed, not just toast text",
        "Use page.locator with regex for flexible toast matching: page.locator(/You're now following.*/, { exact: false })",
        "Increase timeout to 15000-20000ms for personalization features",
      ],
    },
  },
