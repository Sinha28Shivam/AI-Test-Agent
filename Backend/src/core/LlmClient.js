...
function isValidEnvVar(val) {
  return !!val && typeof val === 'string' && val.trim() !== '' && !val.startsWith('$(');
}
...
class LlmClient {
  ...
  async _askRaw(prompt, model = null) {
    ...
    if (!useCopilotCliOnly && isValidEnvVar(process.env.AZURE_OPENAI_API_KEY) && isValidEnvVar(process.env.AZURE_OPENAI_ENDPOINT)) {
      if (!trickLogs) {
      ...
      try {
        ...
        console.warn(`[LLM Warning] Azure OpenAI call failed: ${err.message}. Cause:`, err.cause || err);
      }
    }

    // 2. Standard OpenAI API Support
    if (!useCopilotCliOnly && isValidEnvVar(process.env.OPENAI_API_KEY)) {
      if (!trickLogs) {
      ...
      try {
        ...
        console.warn(`[LLM Warning] OpenAI API call failed: ${err.message}. Cause:`, err.cause || err);
      }
    }

    // 3. GitHub Models API Support (direct)
    if (!useCopilotCliOnly && isValidEnvVar(process.env.GITHUB_TOKEN)) {
      if (!trickLogs) {
      ...
      try {
        ...
        console.warn(`[LLM Warning] Direct API call failed: ${err.message}. Cause:`, err.cause || err);
      }
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      child.on('error', (err) => {
        clearTimeout(timeoutId);
        if (err.code === 'ENOENT') {
          const errMsg = 'GitHub CLI (gh) was not found in the path. Please ensure gh is installed and in your PATH, or configure direct API credentials (AZURE_OPENAI_API_KEY, OPENAI_API_KEY, or GITHUB_TOKEN).';
