...
class PlannerAgent {
  ...
  async createPlan(runId, prompt, name = null) {
    ...
    if (sequence) {
      coldStart = false;
      // Replay if verified or candidate
      if (sequence.state === 'verified' || sequence.state === 'candidate') {
        executionMode = 'replay';
        storedSequence = sequence.sequence;
        console.log(`[PlannerAgent] Sequence found in state '${sequence.state}'. Setting mode to REPLAY.`);
      } else {
