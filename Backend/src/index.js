...
export async function bootstrap() {
  ...
  console.log('[Orchestrator] Running environment diagnostics check...');
  console.log(`- USE_COPILOT_CLI: ${process.env.USE_COPILOT_CLI}`);
  console.log(`- TRICK_COPILOT_LOGS: ${process.env.TRICK_COPILOT_LOGS}`);
  console.log(`- AZURE_OPENAI_API_KEY: ${process.env.AZURE_OPENAI_API_KEY ? `set (len: ${process.env.AZURE_OPENAI_API_KEY.length})` : 'NOT set'}`);
  console.log(`- AZURE_OPENAI_ENDPOINT: ${process.env.AZURE_OPENAI_ENDPOINT ? `set (${process.env.AZURE_OPENAI_ENDPOINT})` : 'NOT set'}`);
  console.log(`- AZURE_OPENAI_DEPLOYMENT: ${process.env.AZURE_OPENAI_DEPLOYMENT ? `set (${process.env.AZURE_OPENAI_DEPLOYMENT})` : 'NOT set'}`);
  console.log(`- OPENAI_API_KEY: ${process.env.OPENAI_API_KEY ? `set (len: ${process.env.OPENAI_API_KEY.length})` : 'NOT set'}`);
  console.log(`- GITHUB_TOKEN: ${process.env.GITHUB_TOKEN ? `set (len: ${process.env.GITHUB_TOKEN.length})` : 'NOT set'}`);
  console.log(`- ADO_PAT: ${process.env.ADO_PAT ? `set (len: ${process.env.ADO_PAT.length})` : 'NOT set'}`);
...
function setupPipelineOrchestration() {
  ...
  messageBus.subscribe(EVENTS.PLAN_CREATED, async (plan) => {
    ...
    const mcpBridge = await mcpBridgePool.acquire();
    ...
    try {
      ...
      if (plan.executionMode === 'replay') {
        try {
          ...
          if (replayResult.success) {
            ...
            // Reset browser context for EXPLORE fallback
            await mcpBridge.callTool('browser_close').catch(() => {});
          }
        } catch (err) {
      ...
      if (passed && executionLog) {
        ...
        try {
          ...
          let runResult = await testRunnerAgent.run(specPath);
          ...
          let statsStatus = runResult.testStats?.statusFromStats;
          let isActualFailure =
            statsStatus === 'failed' ||           // all tests failed
            statsStatus === 'partial' ||          // some tests failed
          ...
          while (isActualFailure && healingAttempts < maxHealingAttempts) {
            healingAttempts++;
            console.log(`[Orchestrator] Playwright validation FAILED. Initiating Self-Healing Attempt ${healingAttempts}/${maxHealingAttempts}...`);

            try {
              const healedCode = await healingAgent.heal(
                specPath,
                runResult.stderr || runResult.stdout || 'Unknown Playwright test execution error',
                plan.domain,
                plan.targetUrl
              );

              // Write healed script to disk
              await fs.writeFile(specPath, healedCode, 'utf-8');
              console.log(`[Orchestrator] Healed Playwright script written to: ${specPath}. Re-running validation...`);

              // Re-run static analysis to ensure healed code has syntax checks passed
              const analysisResult = await staticAnalyzerAgent.analyze({
                runId: plan.runId,
                domain: plan.domain,
                scriptPath: specPath,
              ...
              if (!analysisResult.passed) {
                console.error(`[Orchestrator] Healed script failed static analysis check. Issues:`, analysisResult.issues);
                runResult = {
                  success: false,
                  stdout: '',
                  stderr: `Healed script failed static validation: ${JSON.stringify(analysisResult.issues)}`
                };
              } else {
                runResult = await testRunnerAgent.run(specPath);
              }

              // Update run statistics with healed run details
              run.runnerPassed = runResult.success;
              run.runnerDurationMs = runResult.durationMs;
              run.reportPath = runResult.reportPath;
              run.runnerStdout = runResult.stdout;
              ...
              statsStatus = runResult.testStats?.statusFromStats;
              isActualFailure =
                statsStatus === 'failed' ||
                statsStatus === 'partial' ||
                (!statsStatus && !runResult.success);

              if (!isActualFailure) {
                console.log(`[Orchestrator] Self-Healing Attempt ${healingAttempts} SUCCEEDED! Spec script validated successfully.`);
              }
            } catch (healErr) {
              console.error(`[Orchestrator Error] Self-healing attempt ${healingAttempts} crashed:`, healErr.message);
              break;
            }
          }

          run.healingAttempts = healingAttempts;

          if (isActualFailure) {
            console.error('[Orchestrator] Playwright test runner validation FAILED after healing. Raising issue request...');
            await messageBus.publish(EVENTS.ISSUE_REQUESTED, {
              runId: plan.runId,
              domain: plan.domain,
              coldStart: plan.coldStart,
      ...
      mcpBridge.callTool = originalCallTool;
      await mcpBridgePool.release(mcpBridge);
    }
  });
}
...
function checkGracefulShutdown() {
  if (completedPromptsCount === totalPromptsCount) {
    setTimeout(async () => {
      ...
      // Publish results to Azure DevOps Test Runs
      try {
        const { reportResultsToAdo } = await import('./integration/AdoReporter.js');
        await reportResultsToAdo(completedRuns);
      } catch (adoErr) {
        console.error('[Orchestrator Error] Failed to publish results to Azure DevOps:', adoErr.message);
      }
      ...
      await mcpBridgePool.shutdownAll();
...
async function shutdownRun(runId, outcome) {
  ...
  if (outcome.startsWith('failed') || outcome === 'git_push_failed') {
    ...
    if (stats && stats.statusFromStats) {
      resolvedPassed = stats.statusFromStats === 'passed'; // Only allow fully passed runs
